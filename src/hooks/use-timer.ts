...
export const useTimer = ({ seconds, countDown = true, onFinish }: TimerHookProps) => {
    ...
    if (isActive) {
      const interval = () => {
        if (countDown) {
          ...
          setTimePassedInPercent(Math.ceil((timePassed / 1000 / (seconds + 1)) * 100))
          ...
          setTime(`${minutesLeft <= 9 ? `0${minutesLeft}` : minutesLeft}:${secondsLeft <= 9 ? `0${secondsLeft}` : secondsLeft}`);
        }
      }
